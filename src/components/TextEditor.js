...
const RichTextEditor = ({ mode }) => {
        ...
        if (location.state) {
            ...
            if (content) {
                try {
                    ...
                    setTitle(title);
                    setType(type);
                    fetchImage(`https://artuckeavenuebackend-5.onrender.com${thumbnail}`, filename).then(file => {
    ...
    const insert_completed_article = async (username, contents, title, thumbnail, category, filename, description) => {
        ...
        try {
            const response = await fetch('https://artuckeavenuebackend-5.onrender.com/insert-completed-article/', {
                method: 'POST',
    ...
    const insert_draft_article = async (username, contents, title, thumbnail, category, filename, description) => {
        ...
        try {
            const response = await fetch('https://artuckeavenuebackend-5.onrender.com/insert-draft-article/', {
                method: 'POST',
                body: formData,  // No need for headers, FormData sets them automatically
            });
    ...
    const edit_draft_article = async (article_id, contents, title, thumbnail, category, filename, description) => {
        ...
        try {
            const response = await fetch(`https://artuckeavenuebackend-5.onrender.com/update-draft-article/${article_id}/`, {
    ...
    const edit_completed_article = async (article_id, contents, title, thumbnail, category, filename, description) => {
        ...
        try {
            const response = await fetch(`https://artuckeavenuebackend-5.onrender.com/update-completed-article/${article_id}/`, {
                method: 'POST', // Use PUT or PATCH depending on your backend implementation
                body: formData  
            });

...
import './TestComponentViewSaved.css';
...
const TestComponentViewSaved = ({mode}) => {
    const [savedEntries, setSavedEntries] = useState([]);
    const [expandedIndex, setExpandedIndex] = useState(null);
    const navigate = useNavigate();
    console.log('this is the mode from armans component:', mode)
 
    useEffect(() => {
        // Fetch all saved entries from localStorage
        const fetchArticles = async () => {
            ...
            // Convert API response into the format needed for EditorState
                const entries = response.map(entry => ({
                    ...
                    description: EditorState.createWithContent(
                        convertFromRaw(JSON.parse(entry.description))  // Parse JSON string to object
                    ),
                    editorState: EditorState.createWithContent(
                        convertFromRaw(JSON.parse(entry.contents))  // Parse JSON string to object
                    ),
                }));

                setSavedEntries(entries.reverse());
             } else {
    ...
    const getArticles = async () => {
        try {
            ...
            const response = await fetch('https://artuckeavenuebackend-5.onrender.com/get-drafts-by-username/', {
                method: 'POST',
                headers: {
    ...
    const handleDelete = async (article_id, event) => {
        ...
        try {
            ...
            const response = await fetch(`https://artuckeavenuebackend-5.onrender.com/delete-draft-article/${article_id}/`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            {savedEntries.map((entry, index) => (
                <Card
                    ...
                    style={{
                        marginBottom: '30px',
                        cursor: 'pointer',
                        backgroundColor: mode === 'dark' ? '#000' : '#fff', // Conditional background color
                        color: mode === 'dark' ? 'white' : 'black', // Conditional text color
                    }}
...
export default TestComponentViewSaved;
